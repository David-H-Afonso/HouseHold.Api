...
public static class DishEndpoints
{
    ...
    {
        var group = app.MapGroup("/dish-templates")
            .WithTags("Food")
            .RequireAuthorization();

        group.MapGet("/", async (IDishService service, HttpContext ctx) =>
        {
        ...
        group.MapGet("/{id:guid}", async (Guid id, IDishService service) =>
        {
        ...
        group.MapPost("/", async (CreateDishTemplateRequest request, IDishService service, HttpContext ctx) =>
        {
            var userId = ctx.GetUserId();
            if (userId == null) return Results.Unauthorized();
        ...
        group.MapPut("/{id:guid}", async (Guid id, UpdateDishTemplateRequest request, IDishService service, HttpContext ctx) =>
        {
            var userId = ctx.GetUserId();
            if (userId == null) return Results.Unauthorized();
        ...
        group.MapDelete("/{id:guid}", async (Guid id, IDishService service, HttpContext ctx) =>
        {
            var userId = ctx.GetUserId();
            if (userId == null) return Results.Unauthorized();
