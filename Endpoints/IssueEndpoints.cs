...
public static class IssueEndpoints
{
    ...
    {
        var group = app.MapGroup("/issues").WithTags("Home").RequireAuthorization();

        group
            .MapGet("/", async (IIssueService service) => Results.Ok(await service.GetAllAsync()))
            .WithName("GetIssues");

        group
            .MapGet(
                "/{id:guid}",
                async (Guid id, IIssueService service) =>
                {
        ...
        group
            .MapPost(
                "/",
                async (CreateHomeIssueRequest request, IIssueService service, HttpContext ctx) =>
                {
        ...
        group
            .MapPut(
                "/{id:guid}",
                async (Guid id, UpdateHomeIssueRequest request, IIssueService service) =>
                {
                    if (string.IsNullOrWhiteSpace(request.Title))
                        return Results.BadRequest(new { message = "Title is required." });
