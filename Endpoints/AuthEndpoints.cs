...
public static class AuthEndpoints
{
    public static void MapAuthEndpoints(this WebApplication app)
    {
        ...
        group.MapPost("/login", async (LoginRequest request, IAuthService authService, HttpContext ctx) =>
        {
        ...
        group.MapPost("/refresh", async (RefreshRequest request, IAuthService authService, HttpContext ctx) =>
        {
            var userAgent = ctx.Request.Headers.UserAgent.ToString();
        ...
        .WithName("LogoutAll")
        .WithSummary("Revoke all refresh tokens for the current user (logout from all devices)");
    }
}
