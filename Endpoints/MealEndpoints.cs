...
public static class MealEndpoints
{
    ...
    {
        var group = app.MapGroup("/meal-entries").WithTags("Food").RequireAuthorization();

        group
            .MapGet(
                "/",
                async (IMealService service, HttpContext ctx, DateTime? from, DateTime? to) =>
                {
        ...
        group
            .MapPost(
                "/",
                async (CreateMealEntryRequest request, IMealService service, HttpContext ctx) =>
                {
                    var userId = ctx.GetUserId();
                    if (userId == null)
                        return Results.Unauthorized();
        ...
        group
            .MapPut(
                "/{id:guid}",
                async (Guid id, UpdateMealEntryRequest request, IMealService service, HttpContext ctx) =>
                {
                    var userId = ctx.GetUserId();
                    if (userId == null)
                        return Results.Unauthorized();
