...
public static class TaskEndpoints
{
    public static void MapTaskEndpoints(this WebApplication app)
    {
        ...
        var templates = app.MapGroup("/task-templates")
            .WithTags("Home")
            .RequireAuthorization();

        templates.MapGet("/", async (ITaskService service) =>
            Results.Ok(await service.GetAllTemplatesAsync()))
            .WithName("GetTaskTemplates");

        templates.MapGet("/{id:guid}", async (Guid id, ITaskService service) =>
        {
        ...
        templates.MapPost("/", async (CreateTaskTemplateRequest request, ITaskService service) =>
        {
        ...
        templates.MapPut("/{id:guid}", async (Guid id, UpdateTaskTemplateRequest request, ITaskService service) =>
        {
            if (string.IsNullOrWhiteSpace(request.Title))
                return Results.BadRequest(new { message = "Title is required." });
        ...
        templates.MapDelete("/{id:guid}", async (Guid id, ITaskService service) =>
        {
        ...
        var instances = app.MapGroup("/tasks")
            .WithTags("Home")
            .RequireAuthorization();

        instances.MapGet("/today", async (ITaskService service) =>
            Results.Ok(await service.GetTodayTasksAsync()))
            .WithName("GetTodayTasks")
            .WithSummary("Returns today's task instances (idempotently created) grouped by time slot, plus overdue ones.");

        instances.MapPost("/instances/{id:guid}/complete", async (
            Guid id,
