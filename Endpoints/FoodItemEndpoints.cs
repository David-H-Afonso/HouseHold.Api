...
public static class FoodItemEndpoints
{
    ...
    {
        var group = app.MapGroup("/food-items")
            .WithTags("Food")
            .RequireAuthorization();

        group.MapGet("/", async (IFoodItemService service, string? search) =>
            Results.Ok(await service.GetAllAsync(search)))
            .WithName("GetFoodItems");

        group.MapGet("/{id:guid}", async (Guid id, IFoodItemService service) =>
        {
        ...
        group.MapPost("/", async (CreateFoodItemRequest request, IFoodItemService service, HttpContext ctx) =>
        {
        ...
        group.MapPut("/{id:guid}", async (Guid id, UpdateFoodItemRequest request, IFoodItemService service) =>
        {
            if (string.IsNullOrWhiteSpace(request.Name))
                return Results.BadRequest(new { message = "Name is required." });
        ...
        group.MapDelete("/{id:guid}", async (Guid id, IFoodItemService service) =>
        {
